...
--changeset codex:2
CREATE TABLE IF NOT EXISTS event_journey_steps
(
    id_event_journey_step INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL PRIMARY KEY,
    event                 INTEGER                                  NOT NULL REFERENCES events (id_event) ON DELETE CASCADE,
    journey               INTEGER                                  NOT NULL REFERENCES journeys (id_journey) ON DELETE CASCADE,
    name                  VARCHAR(255)                            DEFAULT NULL,
    position              INTEGER                                  NOT NULL,
    is_current            BOOLEAN                                  NOT NULL DEFAULT FALSE,
    create_date_time      TIMESTAMP                                NOT NULL DEFAULT NOW(),
    update_date_time      TIMESTAMP                                NOT NULL DEFAULT NOW(),
    CONSTRAINT event_journey_steps_event_journey_unique UNIQUE (event, journey),
    CONSTRAINT event_journey_steps_event_position_unique UNIQUE (event, position)
);

CREATE UNIQUE INDEX IF NOT EXISTS event_journey_steps_current_unique
    ON event_journey_steps (event)
    WHERE is_current = TRUE;

INSERT INTO event_journey_steps (event, journey, name, position, is_current)
SELECT id_event, journey, NULL, 1, TRUE
FROM events
WHERE journey IS NOT NULL;

ALTER TABLE events
    DROP CONSTRAINT IF EXISTS events_journey_fkey;

ALTER TABLE events
    DROP COLUMN IF EXISTS journey;

--changeset codex:3
CREATE TABLE IF NOT EXISTS event_participation_step_journeys
(
    id_event_participation_step_journey INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL PRIMARY KEY,
    event_participation                  INTEGER                                  NOT NULL REFERENCES event_participations (id_participation) ON DELETE CASCADE,
    event_journey_step                   INTEGER                                  NOT NULL REFERENCES event_journey_steps (id_event_journey_step) ON DELETE CASCADE,
    journey                              INTEGER                                  NOT NULL REFERENCES users_journeys (id_user_journey) ON DELETE CASCADE,
    create_date_time                     TIMESTAMP                                NOT NULL DEFAULT NOW(),
    update_date_time                     TIMESTAMP                                NOT NULL DEFAULT NOW(),
    CONSTRAINT event_participation_step_journeys_unique UNIQUE (event_participation, event_journey_step)
);

ALTER TABLE users_events_positions
    ADD COLUMN IF NOT EXISTS journey_step INTEGER DEFAULT NULL REFERENCES event_journey_steps (id_event_journey_step) ON DELETE SET NULL;

--changeset codex:4
ALTER TABLE users_journeys
    ADD COLUMN IF NOT EXISTS event INTEGER DEFAULT NULL REFERENCES events (id_event) ON DELETE SET NULL,
    ADD COLUMN IF NOT EXISTS event_journey_step INTEGER DEFAULT NULL REFERENCES event_journey_steps (id_event_journey_step) ON DELETE SET NULL,
    ADD COLUMN IF NOT EXISTS name VARCHAR(255) DEFAULT NULL;

ALTER TABLE users_journeys
    DROP CONSTRAINT IF EXISTS users_journeys_step_requires_event_ck;

ALTER TABLE users_journeys
    ADD CONSTRAINT users_journeys_step_requires_event_ck
        CHECK (event_journey_step IS NULL OR event IS NOT NULL);

--changeset codex:5
CREATE UNIQUE INDEX IF NOT EXISTS users_journeys_user_event_no_step_unique
    ON users_journeys ("user", event)
    WHERE "user" IS NOT NULL
      AND event IS NOT NULL
      AND event_journey_step IS NULL;

--changeset codex:6
CREATE TABLE IF NOT EXISTS notification_device_tokens
(
    id_notification_device_token INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS notification_device_tokens_user_device_platform_host_uindex
    ON notification_device_tokens ("user", device_id, platform, host);

CREATE INDEX IF NOT EXISTS notification_device_tokens_user_active_uindex
    ON notification_device_tokens ("user", active);
