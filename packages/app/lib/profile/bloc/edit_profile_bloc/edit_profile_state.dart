...
class EditProfileState {
  ...
  const EditProfileState({this.status = EditProfileStatus.initial});

  EditProfileState.state(EditProfileState state) : this(status: state.status);
...
class EditProfileLoadFailure extends EditProfileState {
  ...
  EditProfileLoadFailure(EditProfileState state, {required this.errorMessage})
...
class ResetPasswordFailure extends EditProfileState {
  ...
  ResetPasswordFailure(EditProfileState state, {required this.errorMessage})
    : super.state(state.copyWith(status: EditProfileStatus.error));
}
