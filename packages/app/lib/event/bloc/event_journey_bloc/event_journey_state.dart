...
class EventJourneyState {
  ...
  const EventJourneyState({this.status = EventJourneyStatus.initial});

  EventJourneyState.state(EventJourneyState state) : this(status: state.status);

  EventJourneyState copyWith({EventJourneyStatus? status}) {
    return EventJourneyState(status: status ?? this.status);
  }
}
