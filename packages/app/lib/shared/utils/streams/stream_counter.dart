...
class StreamCounter<T, K> {
  ...
  Stream<StreamValue<T, K>> get stream {
    ...
    return _streamController!.stream
        .doOnListen(() {
          _listenerCount++;
          log('$name is listening: $_listenerCount', name: 'StreamCounter');
        })
        .doOnCancel(() {
          _listenerCount--;
          log('$name is listening: $_listenerCount', name: 'StreamCounter');
          if (_listenerCount == 0) {
            log('$name is closing', name: 'StreamCounter');
  ...
  void add(T value, {K? state}) =>
      _streamController?.add(StreamValue(value, state: state ?? initialState));
